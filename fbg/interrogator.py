...
class Interrogator(object):
    def __init__(self, ip_address="10.0.0.126", port=1852, fbg_props=None):
        ...
        self.max_sample_rate = 2000 # 2000 Hz for Micron Optics sm130
    ...
    def set_channel_gain(self, channel_no, gain):
        self.send_command("SET_CH_GAIN_DB {} {}".format(channel_no, gain))
    
    def set_channel_noise_threshold(self, channel_no, value):
        self.send_command("SET_CH_NOISE_THRESH {} {}".format(channel_no, value))
    ...
    @property
    def ch2_gain(self):
        """Returns channel 2 gain in decibels."""
        self.send_command("GET_CH_GAIN_DB 2")
        return float(self.latest_response)
    @ch2_gain.setter
    def ch2_gain(self, gain):
        self.send_command("SET_CH_GAIN_DB 2 {}".format(gain))
        
    @property
    def ch2_noise_thresh(self):
        self.send_command("GET_CH_NOISE_THRESH 2")
        return float(self.latest_response)
    @ch2_noise_thresh.setter
    def ch2_noise_thresh(self, val):
        self.send_command("SET_CH_NOISE_THRESH 2 {}".format(val))
        
    @property
    def ch3_gain(self):
        """Returns channel 2 gain in decibels."""
        self.send_command("GET_CH_GAIN_DB 3")
        return float(self.latest_response)
    @ch3_gain.setter
    def ch3_gain(self, gain):
        self.send_command("SET_CH_GAIN_DB 3 {}".format(gain))
        
    @property
    def ch3_noise_thresh(self):
        self.send_command("GET_CH_NOISE_THRESH 3")
        return float(self.latest_response)
    @ch3_noise_thresh.setter
    def ch3_noise_thresh(self, val):
        self.send_command("SET_CH_NOISE_THRESH 3 {}".format(val))
    
    @property
    def ch4_gain(self):
        """Returns channel 2 gain in decibels."""
        self.send_command("GET_CH_GAIN_DB 4")
        return float(self.latest_response)
    @ch4_gain.setter
    def ch4_gain(self, gain):
        self.send_command("SET_CH_GAIN_DB 4 {}".format(gain))
        return float(self.latest_response)
    
    @property
    def ch4_noise_thresh(self):
        self.send_command("GET_CH_NOISE_THRESH 4")
        return float(self.latest_response)
    @ch4_noise_thresh.setter
    def ch4_noise_thresh(self, val):
        self.send_command("SET_CH_NOISE_THRESH 4 {}".format(val))
    ...
    def data_interleave(self, value):
        ...
        self.send_command("SET_DATA_INTERLEAVE {}".format(ivalue))
        try:
            applied = int(self.data_interleave)
        except Exception:
            applied = ivalue
        self.sample_rate = self.max_sample_rate / max(1, applied)
        print(f"[Interrogator] data_interleave requested={ivalue}, applied={applied}, sample_rate_est={self.sample_rate:.1f} Hz")
    ...
    def data_rate_divider(self, value):
        dvalue = max(1, int(value))
        self.send_command("SET_DATA_RATE_DIVIDER {}".format(dvalue))
        self.sample_rate = self.max_sample_rate / dvalue
    ...
    def set_num_averages(self, avgs, channel_no="", sensor_no=""):
        ...
        parts = self.latest_response.decode(errors="ignore").strip().split()
        if not parts or parts[0] != "Setting":
            print("Failed to set number of averages")
            
    def get_data(self):
        if self.stream_data:
            respsize = self.streaming_socket.recv(10)
            ...
            while valid_response == False:
                try:
                    ...
                    new_byte = self.streaming_socket.recv(1)
                    respsize = respsize[1:] + new_byte
            response = self.streaming_socket.recv(respsize)
            if self.stream_iteration < 1:
                token = self.streaming_socket.recv(0)
            else:
                token = response[-8:]
                response = response[:-8]
                self.stream_iteration += 1
                self.streaming_socket.recv(2)
        else:
        ...
        self.data_header = {"Serial number": serial_number,
                            ...
                            "Ch_1 # of peaks": num_dut1_peaks,
                            "Ch_2 # of peaks": num_dut2_peaks,
                            "Ch_3 # of peaks": num_dut3_peaks,
                            "Ch_4 # of peaks": num_dut4_peaks,
    ...
    def create_sensors(self, fbg_props=None):
        if fbg_props is not None:
            self.fbg_properties = fbg_props

        if not self.fbg_properties:
            self.sensors = []
            return

        entries: Iterable[Tuple[str, Dict]] = []
        if isinstance(self.fbg_properties, dict):
            entries = [
                (name, dict(props)) for name, props in self.fbg_properties.items()
            ]
        elif isinstance(self.fbg_properties, list):
            converted = []
            for idx, props in enumerate(self.fbg_properties):
                if not isinstance(props, dict):
                    continue
                props = dict(props)
                name = props.pop("name", f"fbg_{idx+1}")
                converted.append((name, props))
            entries = converted
        else:
            raise ValueError("Unsupported FBG properties format.")

        ordered = sorted(entries, key=lambda item: item[1].get("position", 0))

        self.sensors = [None] * len(ordered)
        normalized_props = {}
        for idx, (name, props) in enumerate(ordered):
            props.setdefault("position", idx)
            sensor = Sensor(name, properties=props)
            self.sensors[idx] = sensor
            normalized_props[name] = props

        self.fbg_properties = normalized_props
    ...
    def do_append_data(self):
        # self.data["timestamp"] = np.array([self.kernel_timestamp])
        # self.data["serial_no"] = np.array([self.data_serial_no])
        # if len(self.data["time"]) == 0:
        #     newtime = 0.0
        # else:
        #     newtime = self.data["time"][-1] 
        # self.data["time"] = np.array([newtime])
        for s in self.sensors:
            self.data[s.name + "_wavelength"] =np.array([ s.wavelength])
    ...
    def disconnect(self):
        try:
            stream_sock = getattr(self, "streaming_socket", None)
